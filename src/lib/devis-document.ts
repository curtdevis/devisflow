...
<head>
<meta charset="UTF-8">
<title>${h(pdfTitle(result.devisNumber, result.client.name))}</title>
<style>
...
export function buildSummaryHtml(row: DevisRow): string {
  const date = new Date(row.created_at).toLocaleDateString("fr-FR", {
    day: "2-digit", month: "long", year: "numeric",
  });
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>${h(pdfTitle(row.devis_number, row.client_name))}</title>
...
export function getHtml(row: DevisRow): string {
