...
export async function GET(request: NextRequest) {
  ...
  const { data: owners } = ownerIds.length
    ? await admin.from("profiles").select("id, plan, created_at, agence_id, member_of, trial_days").in("id", ownerIds)
    : { data: [] as { id: string; plan: string | null; created_at: string; agence_id: string | null; member_of: string | null; trial_days: number }[] };
  ...
  for (const d of dueCount) {
    ...
    if (owner && !coveredByAgence && !coveredByMember && owner.plan !== "paid") {
      if (isTrialExpired(owner.created_at, owner.trial_days)) {
